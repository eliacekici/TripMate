...
import { getCityDestId, saveSpotPlan, searchSpots } from '../services/BookingApi';
...
const SpotsSearchScreen: React.FC<any> = ({ route }) => {
  const [query, setQuery] = useState(route.params?.destination || '');
  const [loading, setLoading] = useState(false);
  const [places, setPlaces] = useState<SpotPlace[]>([]);
  ...
  const doSearch = async () => {
    ...
    try {
      ...
      const results = await searchSpots({ destId: dest.dest_id });
      if (!results || results.length === 0) {
        setMessage('No spots found for this city.');
      }

      setPlaces(results || []);
    } catch (e: any) {
      setMessage(e?.message || 'Error searching for spots.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSavePlace = async (place: SpotPlace) => {
    try {
      await saveSpotPlan({ place, destination: route.params?.destination });
      setMessage('Spot saved!');
    } catch (error) {
      setMessage('Could not save this spot.');
    }
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.searchSection}>
        <Text style={styles.title}>Find spots in {route.params?.destination || ''}</Text>
        ...
        <TouchableOpacity style={styles.button} onPress={doSearch} disabled={loading}>
          <Text style={styles.buttonText}>{loading ? 'Searching...' : 'Search Spots'}</Text>
        </TouchableOpacity>
      </View>
        ...
        renderItem={({ item }) => (
          <View style={styles.item}>
            ...
            <View style={styles.actions}>
              <TouchableOpacity style={[styles.actionButton, { backgroundColor: '#00223D' }]} onPress={() => openInMaps(item.lat, item.lon)}>
...
export default SpotsSearchScreen;
